...
class FastOutputLayer:
    def __init__(self, input_layer_inputs):
        ...
        self.activation_fun = lambda g, mu, r_plus: 2.0 / np.pi * np.arctan(g * (r_plus - mu)) * 0.5 * (np.sign(r_plus - mu) + 1)
    ...
    def process_all_inputs(self):
        for i in range(np.size(self.input_layer_inputs, 0)):
            ...
            for j in range(self.max_optim_iters):
                activation_fun_aux = lambda g, mu: self.activation_fun(self.g, self.mu, self.r_plus)
